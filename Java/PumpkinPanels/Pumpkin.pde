public class Pumpkin {
  ...
  void explodeColor(color c) {
    if ( (this.state & EXPLODE) == 0) {
      ...
      for ( int i = 0; i < this.sizePixels * 20; i+=10 ) {
        /*
         ...
         * has a greater density of points at larger radii, so that's why
         * it's sqrt(random(0, 1)) and not just random(0, 1) :D
         */
        //float randomRadius = (this.sizePixels / 2) * random(0, 1);
        float randomRadius = (this.sizePixels / 2) * sqrt(random(0, 1));
            
        float random_angle_rad = 2 * PI * random(0, 1);
        float randomX = this.x + (randomRadius * cos(random_angle_rad));
        float randomY = this.y + (randomRadius * sin(random_angle_rad));
        Particle p = new Particle(randomX, randomY, c);
        particles.add(p);
      }
  ...
  private void updateBounce() {
    ...
    if ( this.x > pg.width + this.sizePixels ) {
